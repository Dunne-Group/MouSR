...
shinyServer(function(input, output, session) {
  pdf(NULL)
  isolate(users$number <- users$number + 1)
  ######################################################################################################### 
  #######################################################################################################################
  #########################################################################################################
  observeEvent(input$dataSubmit,{
    ...
    if (chFileFormat == "1"){   
      ...
      UploadData <- reactive({
        ...
        if(is.null(file_to_read)){return("Please Upload Transcriptional Data Matrix")}
        return(read.delim(file=file_to_read$datapath,sep=input$Sep1, header=TRUE))})
      
      Uploadtable <- reactive({
        file_to_read2 <- input$file2
        if(is.null(file_to_read2)){return("Please Upload Sample Information and Labels")}
        return(read.delim(file=file_to_read2$datapath,sep=input$Sep1, header=TRUE))})

      ######## Summary ##############################################################################################################
      output$Title <- renderText({
        if(input$dataSubmit)
          isolate({
            no.samples <- length(colnames(UploadData()))-2
      ...
      output$Info <- renderText({
        if(input$dataSubmit)
          isolate({
            paste(as.character(Uploadtable()[,1]), collapse=", " )
          })
      })
      #####################################################################################################################
      output$t1 <- renderText({
        if(input$dataSubmit)
          isolate({
            paste(as.character(colnames(Uploadtable())[2]),sep=""," Label:")
          })
      })
      #####################################################################################################################
      output$t2 <- renderText({
        if(input$dataSubmit)
          isolate({
            paste(as.character(levels(as.factor(Uploadtable()[,2]))), collapse=", ")
          })
      })
      #######################################################################################################################
      output$Summary <- renderTable({
        if(input$dataSubmit)
          isolate({
            no.samples <- length(colnames( UploadData()))-2
            no.gene <- dim( UploadData())[1]
            res.summary <- rbind(no.samples, no.gene)
            rownames(res.summary) <- c("Samples", "Mouse_Ensembl_ID")
      ...
      colorpick <- function(n) {
        colorselection = seq(15, 375, length = n + 1)
        hcl(h = colorselection, l = 65, c = 100)[1:n]
      }
      output$platter <- renderUI({ 
        lev <- sort(unique(input$select)) 
        cols <-  colorpick(length(lev))
        ...
        if (chs == "1"){
          output$PlotDownloadPCA2D <- downloadHandler(
            ...
            content = function(file){
              ...
              # device <- function(..., width, height) grDevices::jpeg(..., width = input$plot_widthpca*4, height = input$plot_heightpca*4, res = 300, quality = 90, units = "px")
      ...
      observeEvent(input$FilterAnalysis,{
        withProgress(message = "Preparing, please wait",
                     {
                       ...
                       main<-cbind(Big1,Big2)
                       main<-cbind(fs[c(1,2)],main)
        ...
        withProgress(message = "DESeq2 Calculations, please wait",
                     {
                       ...
                       DEGs_top <-subset(match ,log2FoldChange> 2 | log2FoldChange< -2)
                       DEGs_top <- subset(DEGs_top, padj < 0.05)  
        ...
        observeEvent(input$Analysis,{
          ...
          withProgress(message = "Updating Plot, please wait",
                       {
                         
                         
                         output$upregulated <- renderText({
                           isolate({
                             upregulated<-match %>% filter(log2FoldChange> as.numeric(input$MSGfc1))
                         ...
                         output$Downregulated <- renderText({
                           isolate({
                             Downregulated<-match %>% filter(log2FoldChange< as.numeric(input$MSGfc))
                             Downregulated<-count(Downregulated%>% filter(padj <as.numeric (input$MSGpval)))
                             paste("Number of Down-regulated genes:", as.numeric(Downregulated$n),sep="")
                           })
                         })
                         
                         DEGs_top <-subset(match,log2FoldChange> as.numeric(input$MSGfc1)| log2FoldChange< as.numeric(input$MSGfc))
                         DEGs_top <- subset(DEGs_top, padj <as.numeric (input$MSGpval))
                         datar$Gene2<-DEGs_top$group
                         t$DEGs_top<- DEGs_top
                         DEGs_top <-subset( t$DEGs_top, select = -c(group,selectedRowID,baseMean,log2FoldChange,lfcSE,stat,pvalue,padj, symbol))
                         colnames(DEGs_top) <- s
                         DEGs_top_scale <-  DEGs_top-rowMeans(DEGs_top) 
                         datar$DEGs_top_scaleupdate5<-DEGs_top_scale
                         col_nameA<- paste(datar$n1, sep="")
                         col_nameB<-paste(datar$n2, sep="")
                         ...
                         datar$DEGs_top_scaleupdate<-DEGs_top_scale
                         ...
                         observe({
                           ...
                           output$heatmapMSGPlot <- renderPlotly({
                             if (chl == "1"){
                               phe<-heatmaply(DEGs_top_scale, scale_fill_gradient_fun = gradient_col,showticklabels = T,dendrogram =as.character(input$Group7comp), main =paste(as.character(input$Name)), margins =c(0,0,50,0),fontsize_col = 14)
                               phe<-phe %>% layout(height = input$plot_heightheat, width = input$plot_widthheat)
                               ...
                               phe<-heatmaply(DEGs_top_scale, scale_fill_gradient_fun = gradient_col,showticklabels=c(TRUE, FALSE),dendrogram =as.character(input$Group7comp), main =paste(as.character(input$Name)),margins =c(0,0,50,0),fontsize_col = 14)
                               phe<-phe%>% layout(height = input$plot_heightheat, width = input$plot_widthheat)
...
################################################################################################################# 
      ...
      observe({
        ...
        else if  (chselectplot== "3"){
          dataselecheat2<- reactiveValues()
          observeEvent(input$Analysis,{
            DEGs_top_scale <- datar$DEGs_top_scaleupdate
           
            DEGs_top_scale1<-cbind(datar$Gene2,DEGs_top_scale)
       
            names(DEGs_top_scale1)[names(DEGs_top_scale1) == "datar$Gene2"] <- "Genes"
       
            output$Heatmapselect = DT::renderDataTable(DEGs_top_scale1,server = FALSE,
                                                       extensions = 'FixedColumns', 
                                                       options = list(pageLength = 10, autoWidth = FALSE,
                                                                      scrollX = TRUE,
                                                                      scrollCollapse = TRUE,
            ...
            observe({
              ...
              output$dotplot = renderPlotly({
                ...
                if ( chselectl  == "1"){
                  ...
                  heat<- heatmaply( DEGs_top_scale[s, , drop = FALSE], scale_fill_gradient_fun = gradient_col,showticklabels = T,dendrogram =as.character(input$Group8comp), main =paste(as.character(input$Nameheat)),fontsize_col = 14,fontsize_row = 14) 
                  heat<-heat %>% layout(height = input$plot_heightselectheat, width = input$plot_widthselectheat)  
                  if  (chselectheat == "1"){
                    heat<- heat %>% config(
                      toImageButtonOptions= list(filename = as.character(input$Nameheat)))
                  ...
                  heat<- heatmaply( DEGs_top_scale[s, , drop = FALSE], scale_fill_gradient_fun = gradient_col,showticklabels=c(TRUE, FALSE),dendrogram =as.character(input$Group8comp), main =paste(as.character(input$Nameheat)),fontsize_col = 14,fontsize_row = 14) 
                  heat<-heat %>% layout(height = input$plot_heightselectheat, width = input$plot_widthselectheat)  
        ...
        if  (chzscore== "2"){
          if  (chselectplot== "1"){
            ...
            dataselecheat2<- reactiveValues()
            observeEvent(input$Analysis,{
              DEGs_top_scaleup <- datar$DEGs_top_scaleupdate5
              
              
              if (as.numeric(length(as.character(input$Group1comp)))==1){
                names(DEGs_top_scaleup)[1:datar$lg1-1]<-as.character(input$Group1comp)[1]
                gname<-(as.character(input$Group1comp)[1])
                c1<-data.frame( gname=rowMeans(DEGs_top_scaleup[1:datar$lg1-1]))
                names(c1)[names(c1) == "gname"] <- as.character(input$Group1comp)[1]
                Big3<-c1
              }
                ...
                names(DEGs_top_scaleup)[(datar$lg1):datar$lg2-1]<-as.character(input$Group1comp)[2]
                gname2<-(as.character(input$Group1comp)[2])
                c2<-data.frame(gname2 =rowMeans(DEGs_top_scaleup[(datar$lg1):datar$lg2-1]))
                names(c2)[names(c2) == "gname2"] <- as.character(input$Group1comp)[2]
                Big3<-cbind(c1,c2)
              }
                ...
                names(DEGs_top_scaleup)[ (datar$lg1+datar$lg2):datar$lg3-1]<-as.character(input$Group1comp)[3]
                gname3<-(as.character(input$Group1comp)[3])
                c3<-data.frame(gname3 =rowMeans(DEGs_top_scaleup[(datar$lg1+datar$lg2):datar$lg3-1]))
                names(c3)[names(c3) == "gname3"] <- as.character(input$Group1comp)[3]
                Big3<-cbind(c1,c2,c3)
              }
                ...
                names(DEGs_top_scaleup)[ (datar$lg1+datar$lg2+datar$lg3):datar$lg4-1]<-as.character(input$Group1comp)[4]
                gname4<-(as.character(input$Group1comp)[4])
                c4<-data.frame(gname4 =rowMeans(DEGs_top_scaleup[ (datar$lg1+datar$lg2+datar$lg3):datar$lg4-1]))
                names(c4)[names(c4) == "gname4"] <- as.character(input$Group1comp)[4]
                Big3<-cbind(c1,c2,c3,c4)
                ...
                names(DEGs_top_scaleup)[(datar$lg1+datar$lg2+datar$lg3+datar$lg4):datar$lg5-1]<-as.character(input$Group1comp)[5]
                gname5<-(as.character(input$Group1comp)[5])
                c5<-data.frame(gname5 =rowMeans(DEGs_top_scaleup[(datar$lg1+datar$lg2+datar$lg3+datar$lg4):datar$lg5-1]))  
                names(c5)[names(c5) == "gname5"] <- as.character(input$Group1comp)[5]
                Big3<-cbind(c1,c2,c3,c4,c5)
              ...
              if (as.numeric(length(as.character(input$Group2comp)))==1){
                names(DEGs_top_scaleup)[names(DEGs_top_scaleup) == "B1"] <- as.character(input$Group2comp)[1]
                number<-as.numeric(match(as.character(input$Group2comp)[1],names(DEGs_top_scaleup)))
                names(DEGs_top_scaleup)[number:(number+datar$lk1-1)]<-as.character(input$Group2comp)[1]
                kname<-(as.character(input$Group2comp)[1])
                d1<-data.frame(kname =rowMeans(DEGs_top_scaleup[number:(number+datar$lk1-1)]))
                names(d1)[names(d1) == "kname"] <- as.character(input$Group2comp)[1]               
                Big4<-d1
                ...
                names(DEGs_top_scaleup)[(number+datar$lk1):(number+datar$lk1+datar$lk2-1)]<-as.character(input$Group2comp)[2]
                kname2<-(as.character(input$Group2comp)[2])
                d2<-data.frame(kname2 =rowMeans(DEGs_top_scaleup[(number+datar$lk1):(number+datar$lk1+datar$lk2-1)]))
                names(d2)[names(d2) == "kname2"] <- as.character(input$Group2comp)[2]  
                Big4<-cbind(d1,d2) 
                ...
                names(DEGs_top_scaleup)[(number+datar$lk1+datar$lk2):(number+datar$lk1+datar$lk2+datar$lk3-1)]<-as.character(input$Group2comp)[3]
                kname3<-(as.character(input$Group2comp)[3])
                d3<-data.frame(kname3 =rowMeans(DEGs_top_scaleup[(number+datar$lk1+datar$lk2):(number+datar$lk1+datar$lk2+datar$lk3-1)]))
                names(d3)[names(d3) == "kname3"] <- as.character(input$Group2comp)[3]  
                Big4<-cbind(d1,d2,d3) 
                ...
                names(DEGs_top_scaleup)[(number+datar$lk1+datar$lk2+datar$lk3):(number+datar$lk1+datar$lk2+datar$lk3+datar$lk4-1)]<-as.character(input$Group2comp)[4]
                kname4<-(as.character(input$Group2comp)[4])
                d4<-data.frame(kname4 =rowMeans(DEGs_top_scaleup[(number+datar$lk1+datar$lk2+datar$lk3):(number+datar$lk1+datar$lk2+datar$lk3+datar$lk4-1)]))
                names(d4)[names(d4) == "kname4"] <- as.character(input$Group2comp)[4]
                Big4<-cbind(d1,d2,d3,d4) 
              }
                ...
                names(DEGs_top_scaleup)[(number+datar$lk1+datar$lk2+datar$lk3+datar$lk4):(number+datar$lk1+datar$lk2+datar$lk3+datar$lk4+datar$lk5-1)]<-as.character(input$Group2comp)[5]
                kname5<-(as.character(input$Group2comp)[5])
                d5<-data.frame(kname5 =rowMeans(DEGs_top_scaleup[(number+datar$lk1+datar$lk2+datar$lk3+datar$lk4):(number+datar$lk1+datar$lk2+datar$lk3+datar$lk4+datar$lk5-1)]))
                names(d5)[names(d5) == "kname5"] <- as.character(input$Group2comp)[5]
                Big4<-cbind(d1,d2,d3,d4,d5)
              }
              ...
              rownames(dd3)  <- rownames(DEGs_top_scaleup)
              ...
              DEGs_top_scale3<-cbind(datar$Gene2,data_subset_norm2)
              names(DEGs_top_scale3)[names(DEGs_top_scale3) == "datar$Gene2"] <- "Genes"
...
################################################################################################################
      ...
      dat <- reactiveValues()
      observeEvent(input$FilterAnalysis,{
        withProgress(message = "Preparing list of Ensembl ID please wait",
                     {
            ...
            withProgress(message = "Preparing HallMark Table and Plot, please wait",
                         {
                           ...
                           datass$ssgsea.mouse <- gsva(as.matrix(Mouse.Data), x,
                                                       min.sz=10, max.sz=Inf, verbose = T, method = 'ssgsea',parallel.sz=0)
...
observeEvent(input$gegoplot,{
  withProgress(message = "Preparing list of GeneOntology-Pathways, please wait...",
     ...
     output$listofgenes<- downloadHandler(
      ...
      observe({  
        widthggsea <- reactive ({ input$plot_widtgsea })
        heightgsea <- reactive ({ input$plot_heightgsea}) 
        output$gseaPlot <- renderPlot(width = widthggsea, height =  heightgsea,{  
        ...
        if (choptiongsea == "1"){
          output$PlotDownloadgsea <- downloadHandler(
            ...
            content = function(file){
              device <- function(..., width, height) grDevices::png(..., width =input$plot_widthgene*4, height =input$plot_heightgene*4, res = 350, units = "px")
